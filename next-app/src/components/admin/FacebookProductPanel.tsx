...
interface PreviewResponse {
  ...
  /** Facebook-only crop rects, keyed by source image URL. */
  ...
  /** Copy for the generated card that leads every post. */
...
/**
 * Per-product Facebook review and publish surface. Mirrors the Instagram panel;
 * the practical differences are that the post carries a clickable shop link,
 * and Remove genuinely deletes the post through the API (Facebook allows it,
 * Instagram does not).
 */
export default function FacebookProductPanel({ productId }: { productId: string }) {
  ...
  // Deletes the prepared slides/post text and un-queues, but KEEPS the lineup,
  ...
  // Copies the SAVED lineup/crops/card setup onto Instagram — drafts stay
  ...
  const copyCuration = async () => {
    ...
    try {
      const res = await fetch('/api/admin/social/copy-curation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productId, from: 'facebook' }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) throw new Error(data?.error || 'Could not copy the setup.');
      showNotice(data?.message ?? 'Copied to Instagram.', true);
    } catch (err) {
  ...
  const saveLineup = async (
    ...
    try {
      const res = await fetch('/api/admin/facebook/images', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          productId,
        ...
        <>
          ...
          <div>
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              ...
              {!lineupDirty && (
                <button
                  ...
                  title="Copy this lineup, crops and card choices to the Instagram post"
            ...
            {isPublished ? (
              <p className="mb-2 text-xs" style={{ color: 'var(--color-on-surface-variant)' }}>
                This post is already live, so its images can no longer be changed here.
              </p>
            ) : null}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {preview.renditionUrls.map((url, index) => (
                  ...
                  >
                    <Image src={url} alt={`Facebook slide ${index + 1}`} fill sizes="80px" className="object-cover" />
          ...
          <div className="flex flex-wrap items-center gap-2 border-t pt-4" style={{ borderColor: 'var(--color-outline-variant)' }}>
            ...
            {isPublished && confirmRemove && (
              <>
                <span className="text-xs font-semibold" style={{ color: 'var(--color-error)' }}>
                  Delete this post from Facebook? Its likes and comments are lost.
                </span>
                <button
                  type="button"
                  onClick={removePost}
