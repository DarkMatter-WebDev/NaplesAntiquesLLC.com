...
export default function EtsySettingsPanel() {
  ...
  const provisionShippingTiers = async () => {
    ...
    try {
      const res = await fetch('/api/admin/etsy/provision-shipping-tiers', { method: 'POST' });
      const data = await res.json().catch(() => null);
      if (!res.ok) throw new Error(data?.error || 'Could not provision shipping tiers.');
      const warningsText = Array.isArray(data?.warnings) && data.warnings.length ? ` ${data.warnings.join(' ')}` : '';
      showNotice(`Shipping tiers ready — ${data.created} created, ${data.updated} updated, ${data.unchanged} unchanged.${warningsText}`);
      await loadStatus();
    } catch (err) {
  ...
  const pushAllPrices = async () => {
    ...
    try {
      ...
      showNotice(
        `Pushed ${totalPushed} price${totalPushed === 1 ? '' : 's'} to Etsy${totalBlocked ? ` · ${totalBlocked} blocked` : ''}${totalFailed ? ` · ${totalFailed} failed` : ''}.`,
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => void provisionShippingTiers()} disabled={provisioningTiers} className="outline-button text-sm">
                {provisioningTiers ? 'Provisioning shipping tiers…' : 'Provision tiered shipping profiles'}
              </button>
              <p className="text-[0.65rem]" style={{ color: 'var(--color-on-surface-variant)' }}>
                Creates/updates one Etsy shipping profile per site shipping tier ($19–$165) and maps listings to
                them by price. The default profile above stays as the fallback. Requires the
              ...
              <div>
                ...
                <p style={{ color: 'var(--color-on-surface-variant)' }}>
                  {!status.priceAutomation.cronSecretConfigured
                    ? 'Not ready: ETSY_CRON_SECRET is missing from the deployed runtime.'
