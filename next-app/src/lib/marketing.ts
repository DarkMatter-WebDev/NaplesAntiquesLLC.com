...
export type AudienceScope = 'subscribers' | 'accounts' | 'all';
...
export function marketingSiteUrl() {
  return (process.env.SITE_URL || 'https://naplesestatejewelry.co').replace(/\/+$/, '');
}
...
function combineSource(existing: MarketingRecipient['source'], incoming: MarketingRecipient['source']) {
