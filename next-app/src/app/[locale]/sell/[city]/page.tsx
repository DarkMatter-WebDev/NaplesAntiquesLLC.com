...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      type: 'website',
      url: `https://naplesestatejewelry.co${isEs ? '/es' : ''}/sell/${area.slug}`,
...
export default async function SellCityPage({ params }: Props) {
  ...
  const canonicalUrl = `https://naplesestatejewelry.co${isEs ? '/es' : ''}/sell/${area.slug}`;
  ...
  const localBusinessLd = {
    ...
    sameAs: ['https://naplesjewelrybuyers.com', 'https://naplesestatejewelry.com'],
  };
  ...
  const breadcrumbLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: isEs ? 'Inicio' : 'Home', item: `https://naplesestatejewelry.co${isEs ? '/es' : ''}` },
      { '@type': 'ListItem', position: 2, name: isEs ? 'Vender' : 'Sell', item: `https://naplesestatejewelry.co${isEs ? '/es' : ''}/sell` },
      ...
      <main className="pt-16">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {whatWeBuy.map((item) => (
              <div key={item.icon} className="rounded-2xl border border-[#d0c5af] bg-white p-7 shadow-[0_14px_38px_rgba(38,28,6,0.05)] transition-all hover:-translate-y-0.5 hover:shadow-xl">
                <span className="material-symbols-outlined mb-4 block text-[#735c00]" style={{ fontSize: '2.5rem', fontVariationSettings: "'FILL' 0, 'wght' 200" }} aria-hidden="true">
                  {item.icon}
                </span>
              ...
              {steps.map((step, i) => (
                <div key={step.icon} className="text-center">
                  <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-[#d4af37]">
                    <span className="material-symbols-outlined text-white" style={{ fontSize: '1.9rem', fontVariationSettings: "'FILL' 1" }} aria-hidden="true">
                      {step.icon}
                    </span>
                  </div>
        ...
        <section className="bg-[#2f3131] py-24 text-center">
          <div className="mx-auto max-w-2xl px-4">
            <span className="material-symbols-outlined mb-6 block text-[#e9c349]" style={{ fontSize: '3.5rem', fontVariationSettings: "'FILL' 1" }} aria-hidden="true">
              verified_user
            </span>
