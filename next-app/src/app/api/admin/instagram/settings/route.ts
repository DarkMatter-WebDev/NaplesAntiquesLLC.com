...
import { getConnection, updateConnection } from '@/lib/instagram/store';
import { INSTAGRAM_MAX_HASHTAGS } from '@/lib/instagram/client';

export const runtime = 'nodejs';

/** Instagram itself allows 100 posts/24h; this is the owner's own drip cap. */
...
export async function PATCH(req: Request) {
  ...
  if (body.baseHashtags !== undefined) {
    ...
    if (tags.length > INSTAGRAM_MAX_HASHTAGS) {
      return NextResponse.json(
        { error: `Instagram allows at most ${INSTAGRAM_MAX_HASHTAGS} hashtags per post.` },
        { status: 400 },
      );
    }
