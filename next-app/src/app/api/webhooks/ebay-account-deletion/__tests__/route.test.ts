...
describe('computeChallengeResponse — exact hash algorithm', () => {
  ...
  it('hex-SHA256s the concatenation challengeCode + verificationToken + endpoint, in that order', async () => {
    ...
    const endpoint = 'https://naplesestatejewelry.co/api/webhooks/ebay-account-deletion';
...
describe('GET /api/webhooks/ebay-account-deletion — challenge echo', () => {
  beforeEach(() => {
    vi.resetModules();
    process.env = { ...ORIGINAL_ENV, EBAY_VERIFICATION_TOKEN: 'test-verification-token', NEXT_PUBLIC_SITE_URL: 'https://naplesestatejewelry.co' };
  });
  ...
  it('returns 400 when challenge_code is missing', async () => {
    ...
    const res = await GET(new Request('https://naplesestatejewelry.co/api/webhooks/ebay-account-deletion'));
  ...
  it('returns 503 when EBAY_VERIFICATION_TOKEN is not configured', async () => {
    ...
    const { GET } = await import('../route');
    const res = await GET(new Request('https://naplesestatejewelry.co/api/webhooks/ebay-account-deletion?challenge_code=abc'));
  ...
  it('returns 200 with the correctly-computed challengeResponse', async () => {
    ...
    const res = await GET(new Request('https://naplesestatejewelry.co/api/webhooks/ebay-account-deletion?challenge_code=abc123'));
...
describe('POST /api/webhooks/ebay-account-deletion — signature-verify failure path', () => {
  ...
  it('returns 412 when the X-EBAY-SIGNATURE header is missing', async () => {
    ...
    const res = await POST(
      new Request('https://naplesestatejewelry.co/api/webhooks/ebay-account-deletion', {
        method: 'POST',
