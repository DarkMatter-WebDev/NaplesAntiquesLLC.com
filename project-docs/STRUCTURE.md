...
| Admin-only viewport zoom lock + phone editor behavior | `src/app/[locale]/admin/layout.tsx` (the `viewport` export — `/admin/*` only, never the public layouts), the `.product-editor-modal` / `.product-editor-footer` block at the end of `globals.css`, and the scroll-lock + `onScroll` handlers in `components/admin/AdminShell.tsx`; guarded by `lib/__tests__/admin-mobile-editor.test.ts`. Rules in `DECISIONS.md` → *"Admin on a phone"* |
