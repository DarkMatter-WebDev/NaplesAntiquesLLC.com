...
| Business-card QR landing page (`/card`, `/es/card`; noindex, not in the sitemap, no site chrome) | `[locale]/card/page.tsx` + `src/components/card/CardTodayHours.tsx`; facts from `business-location.ts` + admin hours; "Read Our Reviews" → `/reviews`; rules in `DECISIONS.md` → *"The /card page"*; guarded by `lib/__tests__/card-page.test.ts` + `reviews-page.test.ts` (labels) |
