...
manual "Submit Order" (unpaid contact-to-buy) button. **Status: sandbox** â€” see the
...
3. **PayPalCheckoutButton** loads the PayPal JS SDK (`components=buttons`, gold pill,
   ...
   - `onCancel`/`onError` â†’ clean message; the order stays unpaid. Nothing is held
   ...
   **Domestic address and shipping hardening (2026-07-23).**
   ...
   `lib/checkout-shipping.ts` is the sole definition for Local Pickup ($0),
   Priority Insured ($45), and Express Overnight Insured ($75). UI labels and
   estimates, the default, server fee lookup and whitelist, and database
   fulfillment mapping consume the same catalog.
